...
function start() {
  ...
  function DownloadJSON() {
    ...
    saveJsonButton.addEventListener("click", () => {
      ...
      const filename = `tasks-report-${Date.now()}.json`;
      ...
      downloadModal.classList.remove("active");
      window.NotificateUser(`The report ${filename} was downloaded`);
      if (window.userAcceptedVoice) {
        window.notificateUserByVoice(`The report was downloaded`);
      }
    });
  }
